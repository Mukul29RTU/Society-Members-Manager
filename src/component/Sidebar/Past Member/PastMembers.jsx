...
const PastMembers = () => {
  ...
  const uniqueWards = ["All", ...new Set(members.map((m) => m["वार्ड_संख्या"]))].filter(Boolean).sort();
  ...
  const filteredMembers = members
    .filter((m) => {
      ...
      const matchesWard = (selectedWard === "All" || m["वार्ड_संख्या"] === selectedWard);
        ...
        <div className="card-header bg-primary text-white p-4 border-0">
          <div className="row g-3 align-items-center">
            <div className="col-12 col-lg-4">
              <h3 className="mb-0 fw-bold">सदस्य</h3>
            ...
            <div className="col-12 col-md-4 col-lg-3 d-flex gap-2">
              <select className="form-select border-0 shadow-none" onChange={(e) => setSelectedWard(e.target.value)}>
                <option value="All">सभी वार्ड</option>
                {uniqueWards.filter((w) => w !== "All").map((w) => (
                  <option key={w} value={w}>वार्ड {w}</option>
                ))}
              </select>
              <button className="btn btn-light" onClick={() => setSortOrder(sortOrder === "asc" ? "desc" : "asc")}>
                <FaSortNumericDown />
              </button>
            </div>
          </div>
                ...
                <tbody>
                  {filteredMembers.map((member, index) => (
                    <tr 
                      ...
                      onClick={() => navigate(`/member/${member["सदस्य_नंबर"]}`, { 
  state: { 
    myData: "past_member" 
  } 
})} 
                    ...
                    >
                      ...
                      <td><span className="badge bg-primary-subtle text-primary">{member["क्रमांक_संख्या"]}</span></td>
                      <td className="fw-bold">{member["नाम"]}</td>
                      <td className="small text-muted">{member["पता"]}</td>
                      <td>{member["संपर्क"]}</td>
                      <td>{member["वार्ड_संख्या"]}</td>
                      <td>{member.email}</td>
                      <td className="text-end pe-4 text-primary"><FaChevronRight /></td>
                    </tr>
                  ))}
                  {filteredMembers.length === 0 && (
                    <tr>
                      <td colSpan="7" className="text-center py-5 text-muted">कोई सदस्य नहीं मिला।</td>
                    </tr>
                  )}
                </tbody>
              </table>
